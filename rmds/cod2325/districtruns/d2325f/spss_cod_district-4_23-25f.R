...
make_ecf_dummies <- function(data, base_ecf = "1R135") {
  ...
  for (lvl in ecf_levels) {
    nm <- paste0("ecf", lvl)
    data[[nm]] <- ifelse(data$ecf == lvl, 1L, 0L)
  }
  data
}

make_time_adjusted_df <- function(df, mod, formula, p_out = 0.10, verbose = TRUE) {
  coef_names <- c("months_1to14", "months_15to24")
  all_coefs  <- coef(mod)
  ...
  rate1_initial <- unname(ifelse(is.na(time_coefs["months_1to14"]),  0, time_coefs["months_1to14"]))
  rate2_initial <- unname(ifelse(is.na(time_coefs["months_15to24"]), 0, time_coefs["months_15to24"]))
  
  df_out <- df %>%
    dplyr::mutate(
      rate1 = rate1_initial, rate2 = rate2_initial,
      price_index = (1 + rate1)^months_1to14 * (1 + rate2)^months_15to24
    )
