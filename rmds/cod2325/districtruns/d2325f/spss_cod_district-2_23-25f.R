...
backward_pkm <- function(formula, data, p_out = 0.10, verbose = TRUE) {
  ...
  # Terms to always keep so their coefficients remain visible
  keep_terms <- c("months_1to9", "months_19to24")
  repeat {
    s <- summary(mod)$coefficients
    if (nrow(s) <= 1) break
    
    # Drop intercept and protected terms from elimination
...
backward_p <- function(formula, data, p_out = 0.10, verbose = TRUE) {
  ...
  repeat {
    s <- summary(mod)$coefficients
    if (nrow(s) <= 1) break
    # Drop intercept from elimination
...
make_time_adjusted_df <- function(df, mod, formula, p_out = 0.10, verbose = TRUE) {
  coef_names <- c("months_1to18", "months_19to24")
  all_coefs  <- coef(mod)
  ...
  rate1_initial <- unname(ifelse(is.na(time_coefs["months_1to18"]),  0, time_coefs["months_1to18"]))
  rate2_initial <- unname(ifelse(is.na(time_coefs["months_19to24"]), 0, time_coefs["months_19to24"]))
  
  df_out <- df %>%
    dplyr::mutate(
      rate1 = rate1_initial, rate2 = rate2_initial, 
      price_index = (1 + rate1)^months_1to18 * (1 + rate2)^months_19to24)
...
# Keep only terms that exist in the data
