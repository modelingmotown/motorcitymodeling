...
safe_log <- function(x, warn = FALSE, name = NULL) {
  out <- rep(NA_real_, length(x))
  ok <- !is.na(x) & is.finite(x) & x > 0
  if (warn) {
    n_bad <- sum(!ok)
    if (n_bad > 0) {
      var_name <- if (!is.null(name)) paste0(" for ", name) else ""
      warning(
        n_bad, " value(s)", var_name, " were NA, non-finite, zero, or negative before log transform."
      )
    }
  }
  out[ok] <- log(x[ok])
...
#lnsbldsfmed = safe_log(
#  sresb_floorarea - 1104,
#  warn = TRUE,
#  name = "sresb_floorarea - 1104"
#)


weighted_mean_ratio <- function(pred, actual) {
...
# Approximate PRB implementation commonly used in ratio studies.
# This is not a byte-for-byte SPSS reproduction but is standard and usable.
...
add_influence_metrics <- function(data, mod, run_id = 1) {
  ...
  if (!is.null(inv_cov_x)) {
    data[[mah_name]][rows_used] <- as.numeric(
      stats::mahalanobis(X, center = center, cov = cov_x)
    )
  }
  data
}
...
make_ecf_dummies <- function(data, base_ecf = "1r135") {
  if (!"secf" %in% names(data)) {stop("Column 'secf' not found in data.")}
  data$secf <- tolower(trimws(as.character(data$secf)))
  ecf_levels <- sprintf("1r%03d", 101:137)
  ecf_levels <- setdiff(ecf_levels, tolower(base_ecf))
  missing_levels <- setdiff(ecf_levels, unique(data$secf))
  if (length(missing_levels) > 0) {
    message("These ECF levels were not found in secf: ", paste(missing_levels, collapse = ", "))
  }
  for (lvl in ecf_levels) {
    nm <- paste0("ecf", lvl)
    data[[nm]] <- as.integer(data$secf == lvl)
  }
  data
}

make_time_adjusted_df <- function(df, mod, formula, p_out = 0.10, verbose = TRUE) {
  coef_names <- c("months_1to9", "months_10to18", "months_19to24")
  all_coefs  <- coef(mod)
  ...
  rate2_initial <- unname(ifelse(is.na(time_coefs["months_10to18"]), 0, time_coefs["months_10to18"]))
  rate3_initial <- unname(ifelse(is.na(time_coefs["months_19to24"]), 0, time_coefs["months_19to24"]))
  
  df_out <- df %>%
    dplyr::mutate(
      rate1 = rate1_initial, rate2 = rate2_initial, rate3 = rate3_initial,
      price_index = (1 + rate1)^months_1to9 * (1 + rate2)^months_10to18 *
        (1 + rate3)^months_19to24
    )
...
model_metrics_b <- function(model, title, digits = 4) {  
...
summarize_extreme_betas_kable <- function(
    ...
    model_names = c("M1", "M2", "M3", "M4", "M5", "M6"), top_n = 3, bottom_n = 3,
    include_intercept = FALSE, digits = 3,
  ...
  all_extremes <- lapply(seq_along(mods), function(i) {
    ...
    top_part <- df_i %>% dplyr::arrange(dplyr::desc(beta), term) %>% dplyr::slice_head(n = top_n)
    bottom_part <- df_i %>% dplyr::arrange(beta, term) %>% dplyr::slice_head(n = bottom_n)
    dplyr::bind_rows(top_part, bottom_part) %>% dplyr::distinct(term, model, .keep_all = TRUE)
  })
  ...
  list(extreme_rows = extreme_df, summary_data = summary_df, kable = kb)
}
