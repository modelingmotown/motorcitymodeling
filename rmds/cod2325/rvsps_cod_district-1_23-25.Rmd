---
title: "Linear Regression Models for Council District I (2023 - 2025 Sales)"
sub: "R & SPSS Metrics Comparison"
author: |
...
```{r data_import}
df <- read_sav(here("data/city", "CCD1_2023-2025_Data.sav")) |> rename_with(tolower)
```
...
```{r data_prep1}
df <- df |> 
  mutate(priceclass = case_when(
          saleprice < 45000 ~ 1, saleprice >= 45000 & saleprice < 80000 ~ 2,
          saleprice >= 80000 & saleprice < 115000 ~ 3,
          saleprice >= 115000 & saleprice < 180000 ~ 4,
          saleprice >= 180000 & saleprice < 250000 ~ 5, saleprice >= 250000 ~ 6, TRUE ~ 0)
  ) |> filter(sresb_style %in% c('SINGLE FAMILY', '1/2 DUPLEX', 'ROW HOUSE'),
          stotalsqft >= 400, sresb_floorarea >= 400, saleprice >= 10000) |> 
  arrange(parcelno, saleprice, saledate) %>% group_by(parcelno) %>%
  slice_tail(n = 1) %>%  ungroup() %>%
  mutate(priceisasmt_ratio = avwhensold / saleprice) %>%
  filter(!(priceisasmt_ratio >= 0.90 & priceisasmt_ratio <= 1.10)) %>%
  mutate(ratio = sesttcv / saleprice, sppsf = saleprice / sresb_floorarea)
```

```{r data_prep2}
df <- df %>%
  mutate(    # Property Type
    ...
    # Building Eras
    erabuilt = case_when(
      sresb_yearbuilt < 1910 ~ 1, sresb_yearbuilt >= 1910 & sresb_yearbuilt < 1930 ~ 2,
      sresb_yearbuilt >= 1930 & sresb_yearbuilt < 1946 ~ 3,
      sresb_yearbuilt >= 1946 & sresb_yearbuilt < 1961 ~ 4, # Base
      sresb_yearbuilt >= 1961 & sresb_yearbuilt <= 1990 ~ 5, sresb_yearbuilt > 1990 ~ 6, TRUE ~ 0
    ),
    yb_pre_1910     = ifelse(erabuilt == 1, 1, 0), yb_1910_to_1929 = ifelse(erabuilt == 2, 1, 0),
    yb_1930_to_1945 = ifelse(erabuilt == 3, 1, 0), yb_1961_to_1990 = ifelse(erabuilt == 5, 1, 0),
    yb_post_1990    = ifelse(erabuilt == 6, 1, 0),
    
    # Condition (Base is 3 / avg)
    good    = ifelse(scond == 2, 1, 0), fair = ifelse(scond == 4, 1, 0),
    poor    = ifelse(scond == 5, 1, 0), unsound = ifelse(scond == 7, 1, 0),
    ...
    qbelow_avg = ifelse(sresb_bldgclass == 1, 1, 0), qpoor = ifelse(sresb_bldgclass == 0, 1, 0),
    ...
    hot_water      = ifelse(sresb_heat == 2, 1, 0), wall = ifelse(sresb_heat %in% c(6, 8), 1, 0),
    central_air    = ifelse(sresb_heat == 9, 1, 0),
    
    # Building Size (Base is 3 / avg)
    size = case_when(
      sresb_floorarea < 830 ~ 1, sresb_floorarea >= 830 & sresb_floorarea < 1018 ~ 2,
      sresb_floorarea >= 1018 & sresb_floorarea < 1266 ~ 3,
      sresb_floorarea >= 1266 & sresb_floorarea < 1659 ~ 4, sresb_floorarea >= 1659 ~ 5, TRUE ~ 0
    ),
    smallest = ifelse(size == 1, 1, 0), small = ifelse(size == 2, 1, 0),
    large    = ifelse(size == 4, 1, 0), largest = ifelse(size == 5, 1, 0)
  )
...
df <- df %>%
  mutate(
    ...
    months = case_when(
      syear == 2023 ~ smonth - 3, syear == 2024 ~ smonth + 9, syear == 2025 ~ smonth + 21, TRUE ~ 0
    ),
    months_1to9 = ifelse(months > 9, 9, months),
    months_10to18 = case_when(
      months > 18 ~ 9, months > 9 ~ months - 9, TRUE ~ 0
    ), months_19to24 = ifelse(months > 18, months - 18, 0), ln_sppsf = log(sppsf),
    lnsbldsf = log(sresb_floorarea), sbldsfint = sresb_floorarea - 541,
    ...
    base_bldgsize = case_when(
      sresb_style == 'SINGLE FAMILY' ~ 1108, sresb_style == '1/2 DUPLEX' ~ 842,
    ...
    base_lotsize_ratio = stotalsqft / base_lotsize, landratio = stotalsqft / base_lotsize,
    landratio2 = case_when(
      landratio >= 0.90 & landratio <= 1.10 ~ 1,
      sresb_style != 'SINGLE FAMILY' & landratio > 3 ~ 3,
      sresb_style == 'SINGLE FAMILY' & landratio > 4 ~ 4, TRUE ~ landratio
